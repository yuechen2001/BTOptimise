...
describe("POST /api/sessions", () => {
    ...
    it("sets deferredIncomeAssessment=true for student", async () => {
        const res = await createSession({ ...validProfile, employmentStatus: "student", monthlyIncome: 0 });
    ...
    it("sets deferredIncomeAssessment=true for nsf", async () => {
        const res = await createSession({ ...validProfile, employmentStatus: "nsf", monthlyIncome: 0 });
...
describe("PATCH /api/sessions/:sessionId", () => {
    ...
    it("re-derives deferredIncomeAssessment when employmentStatus changes back to employed", async () => {
        const createRes = await createSession({ ...validProfile, employmentStatus: "nsf", monthlyIncome: 0 });
        const { sessionId } = createRes.body.data;
