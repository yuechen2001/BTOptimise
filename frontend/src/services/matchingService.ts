...
function classify(
  ...
  if (cashBuffer < 0.20 || msrUsed > 0.25) {
    const reasons: string[] = [];
    if (cashBuffer < 0.20) reasons.push(`Cash buffer is only ${(cashBuffer * 100).toFixed(0)}%`);
...
export function matchProjects(
  ...
  for (const project of catalogue) {
    ...
    for (const flatOpt of project.flatOptions) {
      ...
      const loanAmount = Math.round(effectivePrice * 0.80);
      const cappedLoan = Math.min(loanAmount, maxLoan.maxLoanAmount);
      const r = 0.026 / 12;
      const n = 25 * 12;
      const monthlyPmt = cappedLoan > 0
