...
const MAX_MSR = 0.3; // 30 % of gross monthly income
const MAX_LOAN_TENURE_YEARS = 25;
const HDB_LTV = 0.8; // max 80 % Loan-to-Value
...
export const PAYMENT_MILESTONES: { stage: string; pct: number }[] = [
    ...
    { stage: 'HDB Loan Drawdown', pct: 0.0 }, // remaining funded by loan
];
...
export function computeEHG(profile: UserProfile): GrantResult {
    ...
    if (assessedIncome > 9000) {
        ...
        breakdown.push(
            `EHG quantum: $${ehg.toLocaleString()} (assessed income $${assessedIncome.toLocaleString()}/mth)`
        );
    }
...
export function computeMilestones(
    ...
    const loanAmt = Math.round(effectivePrice * 0.8);
...
export function checkIncomeEligibility(profile: UserProfile): {
    eligible: boolean;
    reason: string;
} {
    const grossIncome = profile.monthlyIncome + (profile.partnerMonthlyIncome ?? 0);
